...
def _json_default(obj: Any) -> Any:
    """Fallback serializer for JSON columns.

    Python's default json.dumps cannot handle datetime / date / Decimal /
    UUID / bytes objects. Excel cells commonly surface date-typed values
    as datetime objects, which poison any JSON column storing raw workbook
    data (e.g. excel_schemas.manifest). This default handles the common
    types safely.
    """
        ...
        with tracer.start_as_current_span("database_initialize") as span:
            ...
            try:
                ...
                engine_kwargs: dict[str, Any] = {
                    "echo": self._echo,
                    "future": True,
                    # Custom JSON serializer handles datetime / Decimal / UUID
                    # which appear in excel_schemas.manifest from raw cell values.
